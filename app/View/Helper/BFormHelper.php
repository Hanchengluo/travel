...
class BFormHelper extends Helper {
    ...
    var $formatInput = '<div class="control-group"><label class="control-label">%s</label><div class="controls">%s</div></div>';

    function create($model = null, $options = array()) {
        $options = array_merge($options, array('class'=>'form-horizontal'));
        return $this->Form->create($model, $options);
    }

    function input($fieldName, $options = array()) {
        $label = isset($options['label']) ? $options['label'] : __($fieldName);
        $input = $this->Form->input($fieldName, array_merge($options, array('div'=>false, 'label'=>false)));
